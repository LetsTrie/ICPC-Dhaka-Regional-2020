...
exports.getAllUsers = async (req, res) => {
  ...
  const participantCount = users.map(user => user.membersInfo.length).reduce((total, val) => { 
    return total + val 
  })
  res.json({
    status: true,
...
exports.updateImageVisibility = async (req, res) => {
  ...
  fs.readFile(galleryFile, 'utf8', function (err, data) {
    if (err) throw err;
    let gallery = JSON.parse(data);
    gallery[gallery.findIndex(img => { return img.name == name })].visibility = visibility
