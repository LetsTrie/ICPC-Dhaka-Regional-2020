...
class SSLCommerz {
   ...
   */
  constructor(config) {
    this.store_mode = this.setSSLCommerzMode(
      typeof config.isSandboxMode === 'undefined' ? true : config.isSandboxMode
    );
    this.store_id = config.store_id;
    this.store_passwd = config.store_passwd;
    this.submit_url =
      'https://' + this.store_mode + '.sslcommerz.com/gwprocess/v4/api.php';
    this.validation_url =
      'https://' +
      this.store_mode +
  ...
  async hash_validate_ipn(post_body) {
    ...
    let hashed_passwd =
      'store_passwd=' +
      crypto.createHash('md5').update(this.store_passwd).digest('hex') +
      '&';
    ...
    let verify_hash = crypto
      .createHash('md5')
      .update(before_verify_hash)
      .digest('hex');
